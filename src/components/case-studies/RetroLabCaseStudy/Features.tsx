  ...
  return (
    <section className="py-12">
      <div className="mb-12">
        ...
        <p className="text-[#FAF6EE]/70 font-light max-w-2xl text-lg">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        {features.map((feature, idx) => (
          <motion.div
            ...
            className={`group bg-white/5 border border-white/10 rounded-2xl p-8 transition-all duration-300 ${feature.border} hover:bg-white/10 cursor-default relative overflow-hidden`}
          >
            <div className={`absolute -top-10 -right-10 w-40 h-40 bg-white/5 rounded-full blur-[50px] transition-all duration-500 group-hover:bg-current ${feature.color} group-hover:opacity-10`}></div>
            
            <feature.icon className={`w-8 h-8 mb-6 ${feature.color}`} />
            <h4 className="font-display text-xl font-bold mb-3">{feature.title}</h4>
            <p className="text-[#FAF6EE]/70 text-sm leading-relaxed">{feature.desc}</p>
          </motion.div>
        ))}
      </div>
    </section>
